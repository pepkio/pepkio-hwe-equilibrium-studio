...
from pepkio_hwe_equilibrium_studio.config import TOOL_ID
from pepkio_hwe_equilibrium_studio.exceptions import PepkioAPIError
from pepkio_hwe_equilibrium_studio.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "hwe-equilibrium-studio"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert inp["input_kind"] == "genotype_counts"
        assert inp["genotype_counts"]["A1_A1"] == 1787
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("mn_blood_group"), label="pytest")
    ...
    assert result.result["verdict"]["hwe_consistent"] is True
...
def test_run_response_error_field(mock_run_response):
    body = {**mock_run_response, "error": {"code": "TOOL_FAILED", "message": "calc failed"}}

    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=body)

    with _make_client(handler) as client:
        with pytest.raises(PepkioAPIError, match="calc failed"):
            client.run({"mode": "calculator"})
